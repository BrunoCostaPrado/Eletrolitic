use eletrolitic::codegen::Codegen;
use eletrolitic::diagnostic::SourceFile;
use eletrolitic::lexer::Lexer;
use eletrolitic::parser::Parser;
use eletrolitic::type_checker::{TypeChecker, env::TypeEnv};
...
fn stress_lexer_all_token_types() {
  ...
  let unknown_count =
    tokens.iter().filter(|t| matches!(t.kind, eletrolitic::token::TokenKind::Unknown(_))).count();
...
fn stress_compile_real_config_file() {
  // Read the actual test/eletrolitic.config.ts file
  let path =
    std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("test").join("eletrolitic.config.ts");
  let src = std::fs::read_to_string(&path).expect("failed to read test/eletrolitic.config.ts");
  let mut lexer = Lexer::new(&src);
  let tokens = lexer.tokenize().to_vec();
  let lex_diags = lexer.into_diagnostics();
  let lex_errors: Vec<_> =
    lex_diags.iter().filter(|d| d.severity == eletrolitic::diagnostic::Severity::Error).collect();
  let source_file = SourceFile::new("eletrolitic.config.ts", &src);
  let mut parser = Parser::new(tokens, source_file);
  let program = parser.parse();
  let parse_diags: Vec<_> = parser
    .diagnostics()
    .iter()
    .filter(|d| d.severity == eletrolitic::diagnostic::Severity::Error)
    .collect();
  let mut checker = TypeChecker::new();
  let mut env = TypeEnv::new();
  checker.check(&program, &mut env);
